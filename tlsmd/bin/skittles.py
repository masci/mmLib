...
def get_res_frac(chain_type, atm_name, res_name):
    ...
    #frac = 0.0
    if chain_type == 0:
        #frac = "%.1f" % ((int(Constants.AMINO_BACKBONE.index(atm_name)) % 3)/3.0)
        frac = "%d" % (10*(int(Constants.AMINO_BACKBONE.index(atm_name)) % 3)/3.0)
    elif chain_type == 1:
        # Constants.NUCLEIC_BACKBONE.count("P")
        #frac = "%.1f" % ((int(Constants.NUCLEIC_BACKBONE.index(atm_name)) % 6)/6.0)
...
if __name__ == "__main__":
    ...
    for pathx in my_walk(path):
        ...
        for u in range(1, len(atom_obj)):
            ...
            #pos1      = atom_obj[u-1].position
            #pos2      = atom_obj[u].position
            x1        = atom_obj[u-1]["x"]
            y1        = atom_obj[u-1]["y"]
            z1        = atom_obj[u-1]["z"]
            x2        = atom_obj[u]["x"]
            ...
            #tls_group = atom_obj[u-1].tls_group
            tls_group = 0
            ...
            try:
                #bond_pos = float(res_num1) + get_res_frac(chain_type, atm_name1, res_name2)
                bond_pos = "%s.%s" % (

...
def log_job_end(jdict):
    ...
    try:
        open(settings.LOG_PATH, "a").write(" ".join(l) + "\n")
    except IOError:
        log_write("ERROR: cannot open logfile %s" % (settings.LOG_PATH))
...
def get_job(webtlsmdd):
    ...
    tlsmd = [settings.TLSMD_PROGRAM_PATH, "-b", "-rANALYSIS" ]

    ## Job ID and webtlsmdd URL
    tlsmd.append("-j%s" % (job_id))
    tlsmd.append("-x%s" % (settings.WEBTLSMDD))
...
def send_mail(job_id):
    webtlsmdd = xmlrpclib.ServerProxy(settings.WEBTLSMDD)
...
def fetch_and_run_jobs_forever():
    log_write("starting webtlsmdrund.py  version %s" % (settings.VERSION))
    log_write("using xmlrpc server webtlsmdd.py at URL.........: %s" % (settings.WEBTLSMDD))

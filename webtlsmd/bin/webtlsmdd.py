...
from tlsmdlib import tls_calcs, email
...
def SetStructureFile(webtlsmdd, job_id, struct_bin):
    ...
    try:
        ...
        return "Unable to change to settings.TLSMD_WORK_DIR = '%s'" % (settings.TLSMD_WORK_DIR)
...
class WebTLSMDDaemon(object):
    ...
    def job_set_pdb_dir(self, job_id, pdb_id):
        directory = os.path.join(settings.WEBTLSMDD_PDB_DIR, pdb_id)
    ...
    def pdb_exists(self, pdbid):
        try:
            f = open(settings.WEBTLSMDD_PDBID_FILE, 'r')
        except IOError:
            # if it doesn't exit create it
            f = open(settings.WEBTLSMDD_PDBID_FILE, 'w+')
    ...
    def set_pdb_db(self, pdbid):
        try:
            f = open(settings.WEBTLSMDD_PDBID_FILE, 'a')
        except IOError:
            return False
        f.write(pdbid + '\n')
        f.close()
...
def daemon_main():
    ...
    webtlsmdd = WebTLSMDDaemon(settings.WEBTLSMDD_DATABASE)    
